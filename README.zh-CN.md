...
1. 新手机安装并登录 `Tailscale`
2. 打开同一个电脑的 Tailscale 地址
3. 输入同一个 `ACCESS_TOKEN`
4. 如果服务重启后 token 变了，就以电脑当前 `.env` 里的值为准
...
- `运行中`：这个会话当前正在执行本轮任务
- `刚完成`：这个会话刚刚完成了一轮回复
- `需要处理`：本轮失败、无有效回复或需要你关注
- `等待权限`：预留给需要权限确认的流程

首页顶部还会有一个非常轻的待处理区，只列你现在最值得点进去的会话。
