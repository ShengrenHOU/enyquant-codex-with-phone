...
```text
请帮我把 GitHub 仓库 `ShengrenHOU/enyquant-codex-with-phone` 配置成电脑和手机都能用的状态。

目标：
1. 从 `https://github.com/ShengrenHOU/enyquant-codex-with-phone.git` 拉取 `develop` 分支。
2. 进入仓库目录 `enyquant-codex-with-phone`。
3. 检查这台电脑是否已安装 Node.js、codex CLI、Tailscale。
4. 如果缺少 `.env`，就从 `.env.example` 创建。
5. 在 `.env` 里至少配置：
   - HOST=0.0.0.0
   - TAILSCALE_ONLY=true
